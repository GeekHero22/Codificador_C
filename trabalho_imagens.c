...
void cortarMatriz(int linhasTot, int colunasTot, int matriz[linhasTot][colunasTot], int linhaInicio, int linhaFim, int colunaInicio, int colunaFim)
{
    ...
    int linhaMeio = linhaInicio + (linhaFim - linhaInicio) / 2;
    int colunaMeio = colunaInicio + (colunaFim - colunaInicio) / 2;
...
int ler_pbm(const char *nome_arquivo, int matriz[MAX_LINHAS][MAX_COLUNAS], int *linhas, int *colunas)
{
    ...
    // Pular comentários
    char linha[256];         // armazena valor
    int achou_dimensoes = 0; // Flag para indicar se já encontramos as dimensões da imagem

    // Ignora as linhas de comentário (que começam com '#')
    while (!achou_dimensoes)
    {
        ...
        // Tenta extrair dois inteiros da linha (colunas e linhas da imagem)
        if (sscanf(linha, "%d %d", colunas, linhas) == 2)
        {
            achou_dimensoes = 1; // Se conseguir, marca como encontrada
        }
    }
    ...
    for (int i = 0; i < *linhas; i++)
    {
        ...
        {
            // Lê cada pixel (0 ou 1) da imagem e armazena na matriz
            if (fscanf(fp, "%d", &matriz[i][j]) != 1)
            {
                fclose(fp); // Em caso de erro, fecha o arquivo
                return 0;
            }
        }
    }
